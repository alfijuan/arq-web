...
export const LessonsList = () => {
  ...
  React.useEffect(() => {
    axios.get(`http://localhost:4000/courses/${params.id}`)
    .then(response => {
  ...
  const deleteObject = (id) => {
    if(window.confirm('¿Estas seguro que queres borrar la clase?')){
      // axios.delete(`http://localhost:4000/courses/${params.id}/lessons/${id}`)
        ...
        <Row>
          <Col>
            ...
            <div className="d-flex justify-content-end my-3">
              <Link to={`/courses/${params.id}/lessons/add`}>
...
export const LessonsABM = () => {
  ...
  const isNew = params.lessonId === "add";
  ...
  React.useEffect(() => {
    ...
    if(!isNew){
      axios.get(`http://localhost:4000/lessons/${params.lessonId}`)
      .then(response => {
  ...
  const submitForm = (e) => {
    e.preventDefault();
    if(!isNew){
      axios.put(`http://localhost:4000/lessons/${params.lessonId}`, lesson)
      .then(response => {
      ...
      axios.post(`http://localhost:4000/courses/${params.id}/lessons`, lesson)
      .then(response => {
        if(response.data.lesson){
